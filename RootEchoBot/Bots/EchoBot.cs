...
namespace RootEchoBot.Bots
{
    public class EchoBot : ActivityHandler
    {
        ...
        public static readonly string ActiveSkillPropertyName = $"{typeof(EchoBot).FullName}.ActiveSkillProperty";
        ...
        private readonly FacilityRecognizer _luisRecognizer;

        public EchoBot(BotFrameworkAuthentication auth, ConversationState conversationState, SkillsConfiguration skillsConfig, SkillConversationIdFactoryBase conversationIdFactory, IConfiguration configuration, FacilityRecognizer luisRecognizer)
        {
        ...
        protected override async Task OnMessageActivityAsync(ITurnContext<IMessageActivity> turnContext, CancellationToken cancellationToken)
        {
            ...
            {
                Dictionary<string, IntentScore> test = new Dictionary<string, IntentScore>(luisResult.Intents);
                foreach (string key in test.Keys)
                {
                    if (key == "GetFacilityDetails")
                    {
                        if (turnContext.Activity.Text.Contains("get me details of baldwin medical"))
                        {
                            await turnContext.SendActivityAsync(MessageFactory.Text("Got it, connecting you to the skill..."), cancellationToken);

                            turnContext.Activity.Text = "covid";
